...
BG = as_rgb(int(0x1e1e2e))
FG = as_rgb(int(0xcad3f5))
COLOR_TAB_INACTIVE = as_rgb(0x313244)
COLOR_TAB_ACTIVE = as_rgb(0xc6a0f5)
COLOR_ICON_BG = as_rgb(0x89b4fa)
...
class Cell:
    ...
    def draw(self, screen: Screen, max_size: int) -> None:
        ...
        screen.cursor.fg = self.icon_color
...
def draw_left(screen: Screen, max_length: int):
    cell = Cell(folder_icon, get_wd, center[active_index].tab, color=COLOR_ICON_BG)
    cell.draw(screen, max_length)

def draw_right(screen: Screen):
    max_size = screen.columns - screen.cursor.x
    time_cell = Cell(time_icon, get_time, color=COLOR_ICON_BG)
    session_cell = Cell(session_icon, get_session, center[active_index].tab, color=COLOR_ICON_BG)
